...
# Import dataset from Illumina, 19 in total
```{r}
geo_set_illumina <- c('GSE19435','GSE19439','GSE19442',"GSE19443","GSE19444","GSE22098","GSE37250","GSE39939","GSE39940","GSE40553","GSE42825","GSE42826","GSE42827","GSE42830","GSE42831","GSE42832","GSE50834","GSE56153","GSE69581")
Illumina_set_RDS <- lapply(geo_set_illumina, function(x) readRDS(paste0(x,'_sobject.RDS')))
names(Illumina_set_RDS) <- geo_set_illumina

x <- Illumina_set_RDS$GSE42832
colData(x)
colData(x)$TBStatus %>% table()
data.frame(tb=colData(x)$TBStatus,notes=colData(x)$Notes) %>% View()
colData(x)$Notes %>% table()
...
Illumina_set_Multi <-  lapply(Illumina_set_RDS, function(x) get_expr_symbol(x))
names(Illumina_set_Multi) <- geo_set_illumina
saveRDS(Illumina_set_Multi,"Illumina_set_Multi.RDS")
```
...
Illumina_set_TBSig <- lapply(Illumina_set_Multi, function(x) get_sobject_TBSig(x))
# Check Disease Status
# subseeting latent and PTB

# Check the dataset that has latent information
TB_status <- lapply(Illumina_set_TBSig, function(x) colData(x)$Disease)
check <- sapply(TB_status, function(x) any(levels(x) %in% c("Latent")))
names(which(check))

# Get final sobject for TB signature analysis
Illumina_set_TBSig_final <- Illumina_set_TBSig[names(which(check))]

# Make the disease status as character variable
for (i in 1:length(Illumina_set_TBSig_final)){
  colData(Illumina_set_TBSig_final[[i]])$Disease <- colData(Illumina_set_TBSig_final[[i]])$Disease %>% as.character() %>% as.factor()
}

lapply(Illumina_set_TBSig_final, function(x) colData(x)$Disease)
Illumina_set_TBSig_final$GSE69581 <- NULL
```
...
ssgsea_result_illumina_list <- lapply(Illumina_set_TBSig_final, 
...
lapply(ssgsea_result_illumina_list, 
...
### GSE19439, GSE19442, GSE19444
```{r,message = FALSE, results = 'hide', fig.height = 5, fig.width=15}
lapply(ssgsea_result_illumina_list[c("GSE19439", "GSE19442", "GSE19444")], function(x)
  signatureBoxplot(inputData = x, 
                 name = "Boxplots of Signatures, ssGSEA", 
                 signatureColNames = names(TBsignatures)[grep("Berry|Sweeney_OD_3",names(TBsignatures))],
                 annotationColName = "Disease", rotateLabels = FALSE,fill_colors = c("#00AFBB", "#E7B800"))
  
  )
```
### GSE37250, GSE40553
```{r,results = 'hide', fig.height = 5, fig.width=15}
lapply(ssgsea_result_illumina_list[c("GSE37250", "GSE40553")], function(x)
  signatureBoxplot(inputData = x, 
                 name = "Boxplots of Signatures, ssGSEA", 
...
### GSE39939, GSE39940
```{r,results = 'hide', fig.height = 5, fig.width=15}
lapply(ssgsea_result_illumina_list[c("GSE39939", "GSE39940")], function(x)
  signatureBoxplot(inputData = x, 
                 name = "Boxplots of Signatures, ssGSEA", 
                 signatureColNames = names(TBsignatures)[grep("Anderson|Sweeney_OD_3",names(TBsignatures))],
                 annotationColName = "Disease", rotateLabels = FALSE,fill_colors = c("#00AFBB", "#E7B800"))
...
auc_pval_result_illumina <- lapply(ssgsea_result_illumina_list, function(x)
  get_pvalue_auc(x, 
