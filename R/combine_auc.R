#' Combine results from the `list` of [SummarizedExperiment::SummarizedExperiment-class] object.
#'
...
#' @param AUC.abs Boolean. If AUC.abs = `TRUE`, return the AUC values from function
#' \code{\link[ROCit]{rocit}}. If AUC.abs = `FALSE`, return the AUC values for `max(AUC, 1-AUC)`.
#' @param BPPARAM An instance inherited from \code{bplappy}.
#' See \code{\link[BiocParallel]{bplapply}} for details.
#' @return A data frame with features including Signatures, P.value,
#' neg10xLog(P.value)
...
#' res <- runTBsigProfiler(input = TB_indian, seAssay = "logcounts",
...
combine_auc <- function(SE_scored_list, annotationColName, signatureColNames,
    ...
    aucs_result_dat$Signature <- base::factor(aucs_result_dat$Signature, levels = Signature_order)
...
.get_auc_stats <- function(SE_scored, annotationColName = "TBStatus",
                           signatureColNames, num.boot = NULL, percent = 0.95,
                           AUC.abs = FALSE) {
