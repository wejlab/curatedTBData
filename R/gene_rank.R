...
get_gsea_rank <- function(data_sobject,TBSig,annotationColNames="TBStatus"){
  ...
  result_sig <- bplapply(names(TBSig), function(signatureColNames){
    index <- na.omit(match(TBSig[[signatureColNames]],row.names(data_sobject_assay)))
    if(length(index)==0){
      gene_position_summary <- data.frame(Disease=colData(data_sobject)[,annotationColNames])
    ...
    if(len==1){
      gene_position_summary <- data.frame(Disease=colData(data_sobject)[,annotationColNames], position)
      return(gene_position_summary)
    }
    gene_position_summary <- data.frame(Disease=colData(data_sobject)[,annotationColNames], t(position))
...
get_rank_boxplot <- function(sig_list,gset){
  if(class(sig_list[[1]])=="data.frame"){
    ...
    p <- ggplot(data_long, aes(x=variable, y=value, fill=Disease)) +
      geom_boxplot() + theme_bw() +
      theme(plot.title = element_text(size=12, face="bold"),legend.title = element_blank(),
            legend.position = "none", axis.title.x=element_blank(),
            axis.text.x = element_text(colour="Black", size=12, hjust = 0.5, angle = 30,face = "bold"),
  ...
  p_boxplot <- lapply(1:length(data_list), function(x){
    ...
    p <- ggplot(data_long, aes(x=variable, y=value, fill=Disease)) +
      geom_boxplot() + ggtitle(names(data_list)[x]) + theme_bw() +
      theme(plot.title = element_text(size=33, face="bold"),legend.title = element_blank(),
            legend.position = "none", axis.title.x=element_blank(),
            axis.text.x = element_text(colour="Black", size=27, hjust = 0.5, angle = 30,face = "bold"),
