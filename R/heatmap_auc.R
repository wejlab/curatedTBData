...
heatmap_auc <- function(combine_dat, GSE_sig = NULL, facet = TRUE,
  ...
  sig_type_temp <- base::unlist(base::lapply(base::strsplit(signatureColNames, "_"),
                                             function(x) x[2]))
  ...
  if (facet) {
    p <- p + ggplot2::facet_grid(.data$sig_typek ~ ., switch = "y", scales = "free",
                                 space = "free")
...
.facet_rect_position <- function(datta, frames) {
  ...
  base::names(frames_list) <- base::lapply(frames_list,
                                           function(x) x$sig_typek[1]) %>%
    base::unlist()
  datta_list <- datta %>%
    dplyr::group_split(.data$sig_typek)
  base::names(datta_list) <- base::lapply(datta_list,
                                          function(x) x$sig_typek[1]) %>%
    base::unlist()
  # Get the correct index in for training dataset
  # change sig_type levels from sub list based on characters in the full list
