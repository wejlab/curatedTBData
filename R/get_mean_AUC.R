...
#' @param num.boot Numeric. Number of bootstrap times. Default is `100`.
#' @param percent A number indicates the percentage of confidence interval.
#' Default is `0.95`.
#' @return A `data frame` contains mean AUC, lower and upper bootstrap confidence interval
...
get_mean_AUC <- function(df, colName, method = c("percentile", "empirical"),
    ...
    df_list <- df %>%
        dplyr::select(.data$Signature, .data$AUC) %>%
    ...
    meanAUC_list <- base::lapply(df_list, function(x) .bootstrap_mean_CI(x, colName, percent,
                                                                         method, num.boot))
