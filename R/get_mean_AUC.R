...
#' The choices are \code{c("percentile","empirical")}. The default is \code{"empirical"}.
#' @param num.boot Numeric. Number of bootstrap times. Default value is \code{100}
#' @param percent A number indicates the percentage of confidence interval
#' Default is value \code{0.95}
#' @return A \code{data.frame} contains mean AUC, lower and upper bootstrap confidence interval
...
get_mean_AUC <- function(df, colName, method = c("percentile", "empirical"),
    ...
    sigInfo <- df_list %>%
        base::vapply(function(x) base::as.character(x$Signature[1]), base::character(1))
    ## Get summarized table and bootstrap 95% Confidence Interval for the mean AUC
    sigInfo <- base::data.frame(Signature = sigInfo)
    meanAUC_list <- base::lapply(df_list, function(x) .bootstrap_mean_CI(x, colName,
                                                                         method,
                                                                         num.boot,
                                                                         percent))
...
.bootstrap_mean_CI <- function(df, colName, method = c("percentile", "empirical"),
    ...
    bootstrapsample <- base::lapply(base::seq_len(num.boot),
                                    function(i) base::sample(x, n, replace = TRUE))
