...
GSE42830_final <- GSE42830_Non_pvalue %>% left_join(OUT_collapse, by=c('ID_REF' = 'PROBEID')) %>% as_tibble()

arrange(GSE42830_final, desc(times))
```

```{r create SummarizedExperiment}
# Create Row data annotation
row_data <- DataFrame(SYMBOL=GSE42830_final$SYMBOL)
...
row.names(characteristic_data_frame) <- colnames(GSE42830_Non_normalized_counts)
