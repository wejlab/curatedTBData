...
GSE42832_final <- GSE42832_Non_pvalue %>% left_join(OUT_collapse, by=c('ID_REF' = "PROBEID")) %>% as_tibble()
...
GSE42832_experimentData <- new('MIAME',
                      ...
                      abstract="Overall Design: There were 5 patients in each group of healthy controls, tuberculosis and sarcoidosis. The cell types were whole blood, PBMCs, monocytes (CD14), T cells (CD8 and CD4) and neutrophils (CD15).",
...
gse <- getGEO("GSE42832", GSEMatrix =  F)
data_title <- sapply(1:length(GSMList(gse)), function(x)  
  GSMList(gse)[[x]]@header$title)
dat_title_new <- gsub(' Sample.*','',data_title)

new_col_info$TBStatus <- TBStatus
new_col_info$Notes <- dat_title_new
