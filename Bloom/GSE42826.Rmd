...
```{r}
urls <- getGEOSuppFiles('GSE42826', fetch_files = FALSE)
url_non_normalized <- as.character(urls$url[2])

temp <- tempfile()
download.file(url_non_normalized,temp)

GSE42826_Non_normalized <- read.delim(gzfile(temp),row.names = 1, header = TRUE) %>% dplyr::select_if(~sum(!is.na(.)) > 0)
dim(GSE42826_Non_normalized)
```

```{r}
# Renaming column names, remove .pVal
GSE42826_Non_pvalue <- GSE42826_Non_normalized[,-grep('.pVal',colnames(GSE42826_Non_normalized))]
GSE42826_Non_normalized_counts <- GSE42826_Non_normalized[,-grep('.pVal',colnames(GSE42826_Non_normalized))]
dim(GSE42826_Non_pvalue)

gse <- getGEO("GSE42826", GSEMatrix =  F)
colnames(GSE42826_Non_pvalue) <- names(GSMList(gse))
colnames(GSE42826_Non_normalized_counts) <- names(GSMList(gse))
colnames(GSE42826_Non_normalized_counts)
```

```{r convert ProbeID to Gene Symbol, cache=TURE}
# Annotation
PROBES <- row.names(GSE42826_Non_pvalue) # Should subsitute to Normalized file???
...
GSE42826_Non_pvalue$ID_REF <- row.names(GSE42826_Non_pvalue)
GSE42826_final <- GSE42826_Non_pvalue %>% left_join(OUT_collapse, by=c('ID_REF'="PROBEID")) %>% as_tibble()

arrange(GSE42826_final, desc(times))
```

```{r create SummarizedExperiment}
# Create Row data annotation
row_data <- DataFrame(SYMBOL=GSE42826_final$SYMBOL)

# Create Metadata
GSE42826_experimentData <- new('MIAME',
                      name="Chole Bloom",
                      ...
                      abstract="Overall design: There were 8 TB, 11 sarcoidosis and 23 healthy controls",
...
row.names(characteristic_data_frame) <- colnames(GSE42826_Non_normalized_counts)

GSE42826_summarizedExperiemnt <- SummarizedExperiment(assays = list(GSE42826_Non_normalized_counts= as.matrix(GSE42826_Non_normalized_counts)), colData = characteristic_data_frame, rowData = row_data, metadata = list(GSE42826_experimentData)); GSE42826_summarizedExperiemnt
```

```{r}
saveRDS(GSE42826_summarizedExperiemnt, ascii=FALSE, file='~/Desktop/RA work/build_TB_data/Signatures/Bloom/GSE42826/GSE42826_summarizedExperiemnt.RDS')
