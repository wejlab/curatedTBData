...
——————————————————————————————————- Use normalized reads instead ---------------------------




# Use normalized reads
## Create Column Data
```{r}
gse <- getGEO("GSE6112", GSEMatrix =  F)

# Create Column Data
# Two characteristics from each sample, because this study is about comparison
data_characteristic_1 <- lapply(1:length(GSMList(gse)), function(x)  
  GSMList(gse)[[x]]@header$characteristics_ch1)
data_characteristic_2 <- lapply(1:length(GSMList(gse)), function(x)  
  GSMList(gse)[[x]]@header$characteristics_ch2)
...
```{r}
data_exprs <- lapply(1:length(GSMList(gse)), function(x)  
  GSMList(gse)[[x]]@dataTable@table)

# check whether they have the same ID_REF
# Expect each data is identical, check!
sapply(1:length(data_exprs), function(x) all(data_exprs[[1]]$ID_REF == data_exprs[[x]]$ID_REF))
# No they are not the same

data_exprs_reduce <- lapply(data_exprs, function(x) x %>% dplyr::select(ID_REF,Intensity1,Intensity2))

for (i in 2:length(data_exprs_reduce)){
  data_exprs_reduce[[1]] <- data_exprs_reduce[[1]] %>% left_join(data_exprs_reduce[[i]], by=c('ID_REF'='ID_REF'))
}

# Rename column names
col_names <- paste0(rep(names(GSMList(gse)),each=2),rep(c("_ch1","_ch2"),36))

GSE6112_normalized <- data_exprs_reduce[[1]]
colnames(GSE6112_normalized)[-1] <- col_names

row.names(GSE6112_normalized) <- GSE6112_normalized$ID_REF

# Remove ID_REF
GSE6112_normalized <- GSE6112_normalized[,-1]

# Re-order dataframe, make it compatible with column Data
index <-sapply(1:ncol(GSE6112_normalized), function(i) which(colnames(GSE6112_normalized) %in% rownames(characteristic_data_frame)[i]))

GSE6112_normalized_counts <- GSE6112_normalized[,index]
...
GPL4475_raw <- getGEO("GPL4475", GSEMatrix =  F)
# ID is unique
GPL4475_annotate <- GPL4475_raw@dataTable@table

row_data <- data_exprs_reduce[[1]] %>% left_join(GPL4475_annotate, by=c("ID_REF"="ID")) 

row_data <- row_data[,-grep("Intensity",colnames(row_data))]
row_data$SYMBOL_NEW <- row_data$`Primary Sequence Name`

row_data <- row_data %>% DataFrame()
```
...
GSE6112_summarizedExperiemnt <- SummarizedExperiment(assays = list(GSE6112_normalized_counts= as.matrix(GSE6112_normalized_counts)), colData = characteristic_data_frame, rowData = row_data, metadata = list(GSE6112_experimentData)); GSE6112_summarizedExperiemnt
