...
celFiles <- list.celfiles(path = tempd, full.names=TRUE);celFiles
...
GPL11532 <- read.delim("~/Desktop/RA work/build_TB_data/Signatures/Walter/GSE73408/GPL11532-32230.txt", comment.char="#", stringsAsFactors = FALSE)

GPL11532$ID <- as.character(GPL11532$ID)
# Reorganize the reference files, remove redundant ID_REF (make each ID unique)
View(data.frame((table(GPL11532$ID))))
# Checked, they are unique
GSE73408_normalized_rma <- GSE73408_normalized_rma %>% as_tibble() %>% mutate(ID_REF=row.names(GSE73408_normalized_rma))
row_data <- GSE73408_normalized_rma %>% left_join(GPL11532, by=c('ID_REF' = 'ID'))
row_data <- row_data[,-grep('GSM',colnames(row_data))]

row_data <- DataFrame(row_data)
row.names(row_data) <- GSE73408_normalized_rma$ID_REF
```
...
colnames(characteristic_data_frame) <- c('Tissue','Gender','TBStatus','Ethnicity', 'Age', 'GeographicalRegion', 'DiabetesStatus','SmokingStatus','BCG')
row.names(characteristic_data_frame) <- colnames(GSE73408_normalized_counts)
head(characteristic_data_frame)
```
...
GSE73408_experimentData <- new('MIAME',
                      ...
                      pubMedIds = '26582831',
                      other=list(Platform = 'Affymetrix Human Gene 1.1 ST Array'))
