...
indx <- sapply(1:length(ID_table$DescriptionID), 
              function(x) which(ID_table$DescriptionID %in% colnames(GSE40553_Non_pvalue)[x]))
colnames(GSE40553_Non_pvalue) <- ID_table$SampleID[indx]
...
# Annotation
PROBES <- row.names(GSE40553_Non_pvalue) # Should subsitute to Normalized file???
...
GSE40553_final <- OUT_collapse %>% left_join(GSE40553_Non_pvalue, by=c("PROBEID" = 'ID_REF')) %>% as_tibble()
...
row_data <- DataFrame(SYMBOL=GSE40553_final$SYMBOL)
...
colnames(characteristic_data_frame) <- c('TBStatus','TimePoint','Tissue','GeographicalRegion')
...
```
```{r}
saveRDS(GSE40553_summarizedExperiemnt, ascii=FALSE, file='~/Desktop/RA work/build_TB_data/Signatures/Blankley/GSE40553/GSE40553_summarizedExperiemnt.RDS')
```