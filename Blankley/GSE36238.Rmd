...
paste0(geo_access,'/',geo_access,'_RAW.tar')
...
GSE36238_normalized_rma <- exprs(rma(GSE36238_data.affy)) # A matrix
...
# Annotation
PROBES <- row.names(GSE36238_normalized_rma) # Should subsitute to Normalized file???
...
GSE36238_final <- OUT_collapse %>% left_join(GSE36238_normalized_rma, by=c("PROBEID" = 'ID_REF')) %>% as_tibble()
...
row.names(characteristic_data_frame) <- colnames(GSE36238_normalized_counts)
head(characteristic_data_frame)
```

```{r Create RowData MetaData and summarized experiment}
# Create Row data annotation
row_data <- DataFrame(SYMBOL=GSE36238_final$SYMBOL)
...
GSE36238_summarizedExperiemnt <- SummarizedExperiment(assays = list(GSEGSE36238_normalized_counts= as.matrix(GSE36238_normalized_counts)), colData = characteristic_data_frame, rowData = row_data, metadata = list(GSE36238_experimentData)); GSE36238_summarizedExperiemnt
