...
Agi_annotate <- read.delim("~/Desktop/RA work/build_TB_data/Signatures/Blankley/GSE28623/GPL4133-12599.txt", comment.char="#",stringsAsFactors = FALSE)
...
Agi_probe <- Agi_annotate %>% as_tibble() %>% distinct(NAME, .keep_all = TRUE)

# Probe Name is not unique
data.frame((table(GSE28623_RNA$genes$ProbeName)))
data.frame((table(Agi_probe$NAME)))

# Creare Row Data
GSE28623_probe <- data.frame(GSE28623_RNA$genes$ProbeName)
colnames(GSE28623_probe) <- "ID_REF"
row_data <- GSE28623_probe %>% left_join(Agi_probe,by=c('ID_REF'='NAME'))
row_data$GENE_SYMBOL[row_data$GENE_SYMBOL==""] <- "Unmapped"
row_data <- DataFrame(row_data)
row.names(row_data) <- GSE28623_RNA$genes$ProbeName
```
...
GSE28623_experimentData_RNA <- new('MIAME',
                      ...
                      pubMedIds = '22046420',
                      other=list(Platform = 'Agilent-014850 Whole Human Genome Microarray 4x44K G4112F (Feature Number version)'))
