...
row_data$SYMBOL_NEW <- row_data$GENE_SYMBOL
row_data$SYMBOL_NEW <- ifelse(row_data$SYMBOL_NEW %in% "", "NA", row_data$SYMBOL_NEW)

View(row_data)
rowData(sobject) <- row_data %>% DataFrame()

saveRDS(sobject,ascii=FALSE,paste0(geo_access,'/',geo_access,'_sobject.RDS'))
```
