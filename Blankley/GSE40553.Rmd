...
new_col_info$GeographicalRegion <- gsub(" patient.*","",new_col_info$GeographicalRegion)
...
# See unique sample size
tt <- colData(sobject) %>% as_tibble() %>%  filter(TBStatus != "Latent")
colData(sobject)$TBStatus %>% table()

tt$Barcode
gsub("\\..*$","",tt$Barcode) %>% unique() %>% length()
```
