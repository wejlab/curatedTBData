...
filePaths = getGEOSuppFiles("GSE19444")
untar('GSE19444/GSE19444_RAW.tar', exdir="GSE19444/GSE19444_Raw")
files <- list.files("GSE19444/GSE19444_Raw", pattern = "txt.*");files
...
GSE19444_Non_normalized_list <-lapply(files[-which(files=='GSM484599_LTB_LON_test009.txt.gz')],
                                      function(x) read.delim(paste0('GSE19444/GSE19444_Raw/',x),  row.names=1, header=TRUE, col.names = c(gsub('_.*','',x),paste0(gsub('_.*','',x),'.Pval'))))

GSE19444_Non_normalized_temp <- do.call(cbind,GSE19444_Non_normalized_list)

GSM484599 <- na.omit(read.delim("GSE19444/GSE19444_Raw/GSM484599_LTB_LON_test009.txt.gz"))
...
GSE19444_final <- OUT_collapse %>% left_join(GSE19444_Non_pvalue, by=c("PROBEID" = 'ID_REF')) %>% as_tibble()
