...
files <- list.files("GSE19443/GSE19443_Raw", pattern = "txt.*");files
GSE19443_Non_normalized_list <- lapply(files, function(x) read.delim(paste0('GSE19443/GSE19443_Raw/',x), row.names=1, header=TRUE, col.names = c(gsub('_.*','',x),paste0(gsub('_.*','',x),'.Pval'))))
...
GSE19443_final <- OUT_collapse %>% left_join(GSE19443_Non_pvalue, by=c("PROBEID" = 'ID_REF')) %>% as_tibble()
