...
# Special handling, different length of files for each sample
...
check_2 <- sapply(1:ncol(GSE22098_2_dat1_ID),function(x) all(GSE22098_2_dat1_ID[,1]==GSE22098_2_dat1_ID[,x]))
...
check_3 <- sapply(1:ncol(GSE22098_2_dat2_ID),function(x) all(GSE22098_2_dat2_ID[,1]==GSE22098_2_dat2_ID[,x]))
...
GSE22098_Non_normalized_2_final <- GSE22098_final_sub1 %>% inner_join(GSE22098_final_sub2, by= c('ID_REF'="ID_REF")) %>% inner_join(GSE22098_final_sub3, by= c('ID_REF'="ID_REF")) %>% as_tibble()
...
GSE22098_final <- GSE22098_Non_pvalue %>% left_join(OUT_collapse, by=c('ID_REF'="PROBEID")) %>% as_tibble()
...
row_data <- DataFrame(SYMBOL=GSE22098_final$SYMBOL)
