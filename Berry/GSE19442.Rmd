...
GSE19442_final <- GSE19442_Non_pvalue %>% left_join(OUT_collapse, by=c('ID_REF'="PROBEID")) %>% left_join(GPL6947_dat, by = c('ID_REF'='ID')) 

row_data <- GSE19442_final %>% dplyr::select(-grep('GSM',colnames(GSE19442_final))) %>% DataFrame()
...
indx <- sapply(1:length(names(GSMList(gse))), function(i) 
  which(names(GSMList(gse)) %in% colnames(GSE19442_Non_normalized_counts)[i]));indx
