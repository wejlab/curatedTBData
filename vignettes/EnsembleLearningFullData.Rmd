...
```{r}
mySelectedSet <- c("Set1", "Set11", "Set13", "Set14","Set15", "Set19")
mySub <- AUC_PTB_OD$AUC_final %>% dplyr::filter(SetName %in% mySelectedSet)
mySubWithMaxAUC <- AUC_PTB_OD$signatureWithMaxAUC %>% dplyr::filter(SetName %in% mySelectedSet)
myOD <- list(AUC_final = mySub, signatureWithMaxAUC = mySubWithMaxAUC)
p_myOD <- show_voilin_plot(myOD, y = 0.68)
```
