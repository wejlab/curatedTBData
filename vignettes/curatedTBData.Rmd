...
```{r access selected curated datasets microarray, warning=FALSE}
...
from `Rpackage("curatedTBData")` database without checking individual object. \
...
subject were also recorded for each study in the `Rpackage("curatedTBData")`. \
...
Finally, the performance of TB signatures across different studies 
...
`Rpackage("curatedTBData")` provides an alternative display such that 
