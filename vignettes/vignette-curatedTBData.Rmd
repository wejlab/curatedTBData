...
ssgsea_PTB_Latent_combine$GSE <- gsub("\\..*","",row.names(ssgsea_PTB_Latent_combine))
```
...
ssgsea_PTB_Latent_combine %>% dplyr::group_by(Signature) %>% dplyr::summarise_all(mean) %>% select(-GSE) %>% DT::datatable()
...
heatmap_auc <- function(combine_dat){
  ...
  hc <- stats::hclust(dd)
  dat_input1 <-dat_input[hc$order, hc$order]

  p <- ggplot(data = reshape2::melt(dat_input1), aes(x=Var1, y=Var2, fill=value)) + 
       geom_tile() +
...
heatmap_auc(ssgsea_PTB_Latent_combine) + ggtitle("Heatmap of AUC for PTB vs. Latent")
```

## Signature for predicting progression{.tabset}

Datasets GSE79362, GSE107993, GSE107994, GSE112104 contain information about latent progression. Next, we consider using gene signatures to predict progression information

### Calculate ssGSEA for selected objects
...
ssgsea_progress_combine$GSE <- gsub("\\..*","",row.names(ssgsea_progress_combine))
```
...
### Summary Table for the performance of each signature{.tabset}

#### Summary table
```{r summary for progression}
ssgsea_progress_combine %>% dplyr::group_by(Signature) %>% dplyr::summarise_all(mean) %>% select(-GSE) %>% DT::datatable()
```

#### Heatmap

```{r,fig.height = 15, fig.width=8}
heatmap_auc(ssgsea_progress_combine) + ggtitle("Heatmap of AUC for predicting TB progression")
```
...
```{r, results = "asis", message = FALSE, fig.height = 30, fig.width=28, fig.wide=TRUE, eval=FALSE}
for (i in names(TBsignatures)){
...
```{r, results = "asis", message = FALSE, warning=FALSE, fig.height = 30, fig.width=28, fig.wide=TRUE, eval=FALSE}
...
```{r Batch corrected of MDP/ssGSEA, cache=TRUE, message = FALSE, warning = FALSE, results = "hide", eval=FALSE}
# Batch correction
# remove GSE74092 becuase it only has about matched gene symbol
# because GSE74092 used quantitative PCR because they did not have enough coverage to capture all genes
...
```{r, results = "asis", message = FALSE, warning = FALSE, fig.wide=TRUE, fig.height = 32, fig.width=32, eval=FALSE}
