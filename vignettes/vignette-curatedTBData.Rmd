...
```{r singscore PTB vs OD}
singscore_PTB_Latent <- bplapply(multi_set_PTB_Latent, function(x,TBsignatures) {
  sobject_new <- x
  rowData(sobject_new) <- NULL
  dat <- assay(x)
  rankData <- singscore::rankGenes(dat)
  score_list <- lapply(TBsignatures, function(i) singscore::simpleScore(rankData, upSet = i, knownDirection = FALSE)$TotalScore)
  score_df <- do.call(cbind,score_list)
  colData(sobject_new) <- DataFrame(colData(sobject_new),score_df)
  
  sobject_new
}, TBsignatures, BPPARAM=param)
