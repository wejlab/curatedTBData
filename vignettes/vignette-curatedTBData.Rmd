...
## Remove samples with unkown TBStatus

```{r}
#object_list_known <- lapply(objects_list, function(x) RemoveSample(x,,experiment_type="assay_reduce"))
```
...
```{r, warning=FALSE}
objects_list1 <-  lapply(objects_list, function(x)
  SubsetSample(x,c("Latent","PTB"))) %>% plyr::compact() 
...
multi_set_PTB_Latent <- lapply(object_match, function(x)
  SubsetSample(x,c("Latent","PTB"), experiment_type="assay_reduce")) %>% plyr::compact() 
...
for (i in names(TBsignatures)){
  ...
  BoxplotTBSig(ssgsea_PTB_Latent1, sig_name = i)
...
for (i in names(TBsignatures)){
  ...
  BoxplotTBSig(MDP_result, sig_name = i)
...
```{r, results = "asis", message = FALSE, fig.height = 28, fig.width=30}
BoxplotTBSig(MDP_result_NULL, sig_name = NULL)
```
...
```{r, results = "asis", message = FALSE, warning = FALSE, fig.height = 30, fig.width=30}
...
TBSignatureProfiler::signatureBoxplot(
               ...
               rotateLabels = FALSE,
               fill_colors = c("#999999", "#E69F00", "#56B4E9", "#00AFBB"))
```
