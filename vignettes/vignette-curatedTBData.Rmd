...
# Obtain p.value, AUC
ssgsea_PTB_Latent_combine <- combine_auc(ssgsea_PTB_Latent, annotationColName = "TBStatus", signatureColNames = names(TBsignatures), num.boot=50)
```

### Boxplot of all scores signatures{.tabset}

```{r Boxplot for each signature PTB vs. Latent, cache=TRUE, results = "asis", fig.height = 30, fig.width=28, fig.wide=TRUE}
...
Evaluate AUCs by taking te means. How about the P value?
```{r suumary table PTB vs. Latent}
ssgsea_PTB_Latent_combine %>% dplyr::group_by(Signature) %>% dplyr::summarise_all(mean) %>% select(Signature, AUC, LowerAUC, UpperAUC) %>% DT::datatable()
...
```{r AUC ridge plot for PTB vs Latent,fig.height = 15, fig.width=8, cache=TRUE}
...
##### Without grouping
```{r PTB vs. Latent heatmap without grouping,fig.height = 12, fig.width=15, fig.wide=TRUE}
...
heatmap_auc(ssgsea_PTB_Latent_combine, GSE_sig, names(TBsignatures), facet = FALSE) + theme(axis.title.x = element_text(angle=45,vjust=0.5,hjust=-0.12,size=13,face="bold")) + ggtitle("Heatmap of AUC for PTB vs. Latent")
```

##### With grouping
```{r PTB vs. Latent heatmap with grouping,fig.height = 12, fig.width=15, fig.wide=TRUE}
# Import signature and data information
heatmap_auc(ssgsea_PTB_Latent_combine, GSE_sig, names(TBsignatures), facet = TRUE) + theme(axis.title.x = element_text(angle=45,vjust=0.5,hjust=-0.12,size=13,face="bold")) + ggtitle("Heatmap of AUC for PTB vs. Latent")
```
...
saveRDS(ssgsea_PTB_OD, "~/Desktop/ssgsea_PTB_OD.RDS")
# ssgsea_PTB_OD <- readRDS("~/Desktop/ssgsea_PTB_OD.RDS") 
...
```{r Boxplot for each signature PTB vs. OD, cache=TRUE, results = "asis", fig.height = 30, fig.width=28, fig.wide=TRUE}
for (i in names(TBsignatures)){
...
```{r AUC ridge plot for PTB vs OD,fig.height = 15, fig.width=8, cache=TRUE}
# Ridge plot for AUCs distribution across datasets
# PTB vs. OD
get_auc_distribution(ssgsea_PTB_OD_combine) + ggtitle("Ridge plot of AUC for PTB vs. OD") + theme(axis.text.x = ggplot2::element_text(colour="Black", size=12, hjust = 0.5, face="bold"),
                 axis.text.y = ggplot2::element_text(size=12, angle = 0, hjust = 0.5))
...
##### Without grouping
```{r PTB vs. OD heatmap without grouping,fig.height = 12, fig.width=10, fig.wide=TRUE}
# Import signature and data information
GSE_sig <- readxl::read_excel("~/Desktop/RA work/build_TB_data/Signatures/Signature_data_cur.xlsx",sheet = "Sheet2")
...
##### With grouping
```{r PTB vs. OD heatmap with grouping,fig.height = 12, fig.width=10, fig.wide=TRUE}
heatmap_auc(ssgsea_PTB_OD_combine, GSE_sig, TBsignatures = names(TBsignatures), facet = TRUE) + ggtitle("Heatmap of AUC for PTB vs. OD")
...
```{r Boxplot for progression, results = "asis", fig.height = 12, fig.width=8, fig.wide=TRUE}
for (i in names(TBsignatures)){
...
```{r progression heatmap without grouping,fig.height = 10, fig.width=6}
...
```{r progression heatmap with grouping,fig.height = 10, fig.width=6}
