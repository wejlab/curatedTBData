...
ssgsea_PTB_Latent <- lapply(multi_set_PTB_Latent,
                    function(x) TBSignatureProfiler::runTBsigProfiler(input = x,
                                  ...
                                  parallel.sz = 14))
# load("~/Desktop/ssgsea_PTB_Latent.RData")
...
for (i in names(TBsignatures)){
  ...
  ssgsea_PTB_Latent_combine %>% dplyr::filter(Signature == i) %>% dplyr::select(-Signature) %>% DT::datatable() %>% print()
...
### Comparison of Signature Performance using AUCs
...
```{r Calculate ssGSEA for progression, cache=TRUE}
...
for (i in gse_progress){
  ...
  ssgsea_progress_combine %>% dplyr::filter(GSE == i) %>% dplyr::select(-GSE) %>% DT::datatable() %>% print()
