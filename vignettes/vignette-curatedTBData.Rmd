...
Recall the list of studies that have control samples. We could either pick up some data from the data for batch correction or use all of them. Next we will use all the available 21 studies
```{r}
# Batch correction
# remove GSE74092 becuase it only has 200+ matched gene symbol
MDP_batch <- CombineObjects(object_match, gse_name = MDP_control_name[-which(MDP_control_name=="GSE74092")]) 
...
```{r, results = "asis", message = FALSE, fig.height = 20, fig.width=28}
# 5 levels of TBStatus: Control/Laent/NA/OD/PTB
# Create SummarizedExperiment Object to make it compaticble with the function

TBSignatureProfiler::signatureBoxplot(inputData = SummarizedExperiment(colData = MDP_batch_result),
                                                  ...
                                                  rotateLabels = FALSE,
                                                  fill_colors = c("#999999", "#E69F00", "#56B4E9", "#FC4E07","#00AFBB"))
```
