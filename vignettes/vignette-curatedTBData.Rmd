...
ssgsea_PTB_Latent <- lapply(multi_set_PTB_Latent,
                    function(x) TBSignatureProfiler::runTBsigProfiler(input = x,
                                  ...
                                  signatures = TBsignatures, # list of Signatures from 
                                  algorithm = "ssGSEA",
...
for (i in TBsig_PTB_Latent){
  ...
  Boxplot_TBSig(ssgsea_PTB_Latent, sig_name = i)
...
Boxplot_TBSig(ssgsea_PTB_Latent[-index_Sloot], sig_name = "Sloot_HIV_2")

```

```{r boxplot for Sloot_HIV_2 PTB vs. Latent, results="hide",fig.height = 16, fig.width=24}

Boxplot_TBSig1(ssgsea_PTB_Latent,gset=TBsignatures, sig_name = "Sloot_HIV_2")
...
Boxplot_TBSig(ssgsea_PTB_Latent[-index_Suliman], sig_name = "Suliman_RISK_4")
...
MDP_result <- lapply(MDP_control[1:8], function(x) MDP(x, gset = TBsignatures, experiment_type = "assay_reduce", assay_name = 1))

p_boxplot <- lapply(1:length(MDP_result), function(x)
    TBSignatureProfiler::signatureBoxplot(inputData = MDP_result[[x]],
                                          name = names(MDP_result)[x],
