...
ssgsea_PTB_Latent_combine$GSE <- row.names(ssgsea_PTB_Latent_combine)
```

### Boxplot of all scores signatures{.tabset}

```{r Boxplot for each signature PTB vs. Latent, results = "asis", fig.height = 30, fig.width=28, fig.wide=TRUE}
for (i in names(TBsignatures)[1:3]){
...
```{r Table for each signature PTB vs. Latent, results = "asis"}
for (i in names(TBsignatures)){

  cat("####", i, "\n")
  
  ssgsea_PTB_Latent_combine %>% filter(Signature == i) %>% dplyr::select(-Signature) %>% mutate(GSE = gsub("\\..*","",GSE)) %>% DT::datatable() %>% print()
...
```{r}
ssgsea_PTB_Latent_combine %>% dplyr::group_by(Signature) %>% dplyr::summarise_all(mean) %>% select(-GSE) %>% arrange(desc(AUC)) %>% DT::datatable()
```
...
Some datasets contain information about latent progression. Next, we consider using gene signatures to predict progression information
```{r Progressor vs. Non-progressor, cache=TRUE}
