...
```{r load list of datasets, message=FALSE}
...
# Subset TBstatus, TBSignatures, and GSE from each SummarizedEpxeriment object
ssgsea_PTB_Latent1 <- SignatureFilter(ssgsea_PTB_Latent, gset = TBsignatures, annotationColName="TBStatus")
names(ssgsea_PTB_Latent1) <- names(ssgsea_PTB_Latent)
```

### Boxplot of all scores signatures{.tabset}

```{r Boxplot for each signature PTB vs. Latent, results = "asis", message = FALSE, fig.height = 30, fig.width=28, fig.wide=TRUE}
...
Table presnets signatures with P.value, 
```{r, cache=TRUE}
for (i in names(ssgsea_PTB_Latent)){

  cat("####", i, "\n")
  
  get_stats(ssgsea_PTB_Latent[[i]], annotationColName = "TBStatus", signatureColNames = names(TBsignatures), num.boot = 50)
...
ssgsea_PTB_Latent_combine <- combine_auc(ssgsea_PTB_Latent, annotationColName = "TBStatus", signatureColNames = names(TBsignatures))
...
### Signature Performance predicting progression
...
BoxplotTBSig(MDP_batch_result, gset = names(TBsignatures), annotationName = "TBStatus")
```

### Boxplot for batch corrected ssGSEA
```{r, results = "asis", message = FALSE, warning = FALSE, fig.wide=TRUE, fig.height = 32, fig.width=32}
BoxplotTBSig(data.frame(colData(MDP_batch_ssgsea)), gset = names(TBsignatures), annotationName = "TBStatus")
```

```{r, results = "asis", message = FALSE, warning = FALSE, fig.wide=TRUE, fig.height = 32, fig.width=32,eval=FALSE,include=FALSE}
BoxplotTBSig(data.frame(kk[,-ncol(kk)]), gset = names(TBsignatures), annotationName = "TBStatus")
