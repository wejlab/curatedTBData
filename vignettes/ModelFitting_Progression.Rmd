...
# theObject <- GSE112104_test_sub
# signatureName <- "Leong_RISK_29"
# theObject <- GSE79362_train_sub
# GSE <- "GSE79362"
progress_heatmap <- list(GSE112104=progress_norm_filter$GSE112104,GSE107994=progress_norm_filter$GSE107994)
GSE79362_train_full <- progress_norm_filter$GSE79362
...
progress_heatmap$GSE79362 <- GSE79362_train_full[,unique(sample_baseline$first)]
progress_heatmap$GSE94438 <- 
plotComplextHeatmap <- function(theObject, annotationColName, signatureName){
     signatureGenes <- TBSignatureProfiler::TBsignatures[[signatureName]]
     dat <- assay(theObject)
     dat_sig <- dat %>% data.frame() %>% 
       dplyr::filter(row.names(dat) %in% signatureGenes) %>% t() %>% data.frame()
     preProc <- preProcess(dat_sig,method = c("center", "scale"))
     dat_sig_scale <- predict(preProc, dat_sig)
     column_ha <- HeatmapAnnotation(group = colData(theObject)[,annotationColName],
                                    col=list(group = c("Positive" = "red",
                                                       "Negative" = "green")))
     dat_heatmap <- t(dat_sig_scale)
     colnames(dat_heatmap) <- NULL
     Heatmap(dat_heatmap, name = NULL, top_annotation = column_ha)
}
plotComplextHeatmap(theObject, annotationColName, signatureName,GSE)
```

## Sweeney_OD_3
```{r, results = "asis", fig.width=12, fig.height = 8}

for (i in seq_len(length(progress_heatmap))){
  cat("####", names(progress_heatmap)[i], "\n")
  
  plotComplextHeatmap(progress_heatmap[[i]], 
                      annotationColName = "Progression",
                      signatureName = "Sweeney_OD_3")
