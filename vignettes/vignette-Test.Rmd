...
```{r load a list of RNA-seq studies}
...
object_list_RNA_seq <- get_curatedTBData(geo_access = geo_RNA_seq$`GEO accession`)
```

## Load a list of RNA-seq data
```{r load a list of microarray studies}
geo_microarray <- DataSummary %>% 
  dplyr::filter(stringr::str_detect(.data$GeneralType, "Microarray")) %>%
  dplyr::select(.data$`GEO accession`)
object_list_microarray <- get_curatedTBData(geo_access = geo_microarray$`GEO accession`)
```
...
GSE107991_mobject_norm <- Normalization(GSE107991_mobject, RNAseq_method = "TMM", experiment_name = "assay_raw")
```
...
boxplot(assays(experiments(GSE107991_mobject_norm)[["assay_raw"]])[["GSE107991_TMM"]][,1:20])
```
...
object_norm <- bplapply(objects_list, function(x) Normalization(x, microarray_method = "quantile", RNAseq_method = "TMM", experiment_name = "assay_raw"), BPPARAM = param) 


# Match Probe to gene symbols
# Use the normalized data from assay_raw by specifying the experiment_name=assay_raw_norm
...
sobject <- CombineObjects(object_match1, list_name = geo_microarray$`GEO accession`, experiment_name = "assay_MatchProbe", batch.adjust = TRUE)

# The samples that correspond to their datasets can be found at:
colData(sobject)[,"GSE"]
...
# Possible outcome of interest could be patient's TB subtypes, this can be found at:
colData(sobject)[,"TBStatus"]
...
multi_set_PTB_Latent <- bplapply(object_match, function(x)
  subset_curatedTBData(x, annotationColName = "TBStatus", annotationCondition = c("Latent","PTB"), experiment_name = "assay_MatchProbe"), BPPARAM = param) %>% plyr::compact()   # Remove empty objects use plyr::compact()
...
```{r Boxplot for each signature rank,include=FALSE, results = "asis", fig.width=50, fig.height = 50, fig.wide=TRUE,eval=FALSE, include=FALSE}
...
ssgsea_PTB_Latent_auc_CI <- cbind(Signature=sapply(PTB_Latent_auc_summary_ssgsea, 
...
# PTB vs. Latent
get_auc_distribution(ssgsea_PTB_Latent_combine) + ggtitle("Ridge plot of AUC for PTB vs. Latent") + theme(axis.text.x = element_text(colour="Black", size=12, hjust = 0.5, face="bold"),
...
heatmap_auc(ssgsea_PTB_Latent_combine, SignatureInfo, 
            names(TBsignatures), facet = TRUE) + ggtitle("Heatmap of AUC for PTB vs. Latent")
```
