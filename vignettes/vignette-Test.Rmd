...
## Load a list of Microarray data
...
# UseAssay specifying which assay to use, 
...
# Make merge study with genes more than 10000. Remove GSE6112 (7658), 
# GSE74092 (281), GSEXXXX (7515)
geo_rna_seq <- DataSummary %>% dplyr::as_tibble() %>%
  dplyr::filter(stringr::str_detect(.data$GeneralType, "RNA-seq")) %>%
  dplyr::select(.data$`GEO accession`)
index_remove <- which(names(object_match1) %in% c("GSE6112", "GSE74092", "GSEXXXX"))
forGabeMerge <- CombineObjects(object_match1, 
                          list_name = names(object_match1)[-index_remove], 
                          experiment_name = "assay_MatchProbe")
forGabe1 <- lapply(object_match1, function(x) {
  x[["assay_raw"]] = NULL
  x
})
saveRDS(forGabe1, "~/Desktop/curatedTBData_list.RDS")
saveRDS(forGabeMerge, "~/Dropbox/Gabe/MergedStudy.RDS")
```
...
```{r calculate ssgsea for PTB vs Latent, cache=TRUE, results = "hide", message=FALSE, warning=FALSE}
...
for (i in names(TBsignatures)){
  ...
  print(htmltools::tagList(DT::datatable(plage_PTB_Latent_combine %>% dplyr::filter(.data$Signature == i) %>% dplyr::select(-.data$Signature))))
