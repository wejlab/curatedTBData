...
filePaths = getGEOSuppFiles("GSE69581")
GSE69581_Non_normalized <- read.delim("GSE69581/GSE69581_non-normalized_data.txt.gz", row.names = 1, header = TRUE)
...
GSE69581_final <- OUT_collapse %>% left_join(GSE69581_Non_normalized, by=c("PROBEID" = 'ID_REF')) %>% as_tibble()
...
```
```{r insepct SummarizedExperiment Object}
colData(GSE69581_summarizedExperiemnt)
```
```{r}
rowData(GSE69581_summarizedExperiemnt)
```
```{r}
metadata(GSE69581_summarizedExperiemnt)
```
```{r}
...
readRDS('~/Desktop/RA work/build_TB_data/Signatures/Esmail/GSE69581/GSE69581_summarizedExperiemnt.RDS')
```
