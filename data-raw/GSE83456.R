...
GSE83456_Non_pvalues <- readRawData(geo, sequencePlatform)
# Remove p-value manually
GSE83456_Non_pvalues <- GSE83456_Non_pvalues[, -c(2, grep('\\.1',colnames(GSE83456_Non_pvalues)))]
...
GSE83456_Non_normalized_counts <- GSE83456_Non_pvalues

##### Create column data #####
characteristic_data_frame <- readRawColData(gse)
colnames(characteristic_data_frame) <- c("Gender", "Ethnicity", "Age", "TBStatus","Tissue")
...
GSE83456_sobject <- SummarizedExperiment::SummarizedExperiment(
  assays = list(GSE83456_Non_normalized_counts= as.matrix(GSE83456_Non_normalized_counts)),
