...
##### Read in raw data #####
urls <- GEOquery::getGEOSuppFiles("GSE19442", fetch_files = FALSE)
...
GSE19442_Non_normalized_list <- lapply(files, function(x)
  read.delim(paste0(tempd,"/", x), header = TRUE,
             col.names = c("ID_REF", gsub("_.*", "", x), paste0(gsub("_.*", "", x), ".Pval")),
             stringsAsFactors = FALSE))
GSE19442_Non_normalized_list_noPvalue <- lapply(GSE19442_Non_normalized_list, function(x)
  x[, -grep(".Pval",colnames(x))])
# Merge list to a matrix based on the ID_REF
GSE19442_Non_normalized <- Reduce(function(x, y)
  merge(x, y, by = "ID_REF", all = FALSE),
  lapply(GSE19442_Non_normalized_list_noPvalue, function(x) {x}))
row.names(GSE19442_Non_normalized) <- GSE19442_Non_normalized$ID_REF
GSE19442_Non_normalized_counts <- GSE19442_Non_pvalue <- GSE19442_Non_normalized[-1]

##### Create Column data #####
gse <- GEOquery::getGEO("GSE19442", GSEMatrix = FALSE)
...
GSE19442_final <- GSE19442_Non_pvalue %>%
  dplyr::left_join(OUT_collapse, by=c("ID_REF" = "PROBEID")) %>%
  dplyr::left_join(GPL6947_dat, by = c("ID_REF" = "ID"))

# Create row data
row_data <- GSE19442_final %>%
  dplyr::select(-grep("GSM", colnames(GSE19442_final))) %>%
  S4Vectors::DataFrame()
...
GSE19442_sobject <- SummarizedExperiment(
  assays = list(GSE19442_Non_normalized_counts= as.matrix(GSE19442_Non_normalized_counts)),
  colData = new_col_info,
  rowData = new_row_data,
  metadata = list(GSE19442_experimentData))
save_raw_files(GSE19442_sobject, path = "data-raw/", geo = "GSE19442")
